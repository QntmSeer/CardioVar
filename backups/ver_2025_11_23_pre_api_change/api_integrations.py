...
def fetch_ensembl_gene(gene_symbol: str, force_live: bool = False) -> Optional[Dict[str, Any]]:
    """Fetch gene information from Ensembl with caching, dynamic links, and fallback."""
    global fallback_used
    ...
    try:
        url = f"{ENSEMBL_API}/lookup/symbol/homo_sapiens/{gene_symbol}?content-type=application/json"
        resp = SESSION.get(url, timeout=15)
        if resp.status_code == 200:
            data = resp.json()
            if "description" not in data:
                data["description"] = f"Gene {gene_symbol}"
            # Add links
            data["links"] = {
                ...
                "gnomAD": f"https://gnomad.broadinstitute.org/gene/{data.get('id', gene_symbol)}?dataset=gnomad_r2_1",
                "Ensembl": f"https://www.ensembl.org/Homo_sapiens/Gene/Summary?g={data.get('id', gene_symbol)}",
    ...
    if force_live:
        logging.warning("Force live Ensembl failed; not attempting fallback.")
        fallback_used = True
        return None

    local = load_fallback_gene_data(gene_symbol)
    if local:
        fallback_used = True
        cache.set(cache_key, local)
        return local
    fallback_used = True
    return None
