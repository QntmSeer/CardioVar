        ...
        with tab1:
            if 'data_sources' in data:
                with st.expander("📊 Data Sources & Provenance"):
                    ...
                    # Highlight Backup API usage
                    sources = data['data_sources']
                    if "MyGene.info" in str(sources.values()) or "MyVariant.info" in str(sources.values()):
                        st.info("ℹ️ Backup APIs (MyGene.info / MyVariant.info) were used for some data elements.")
                    
                    df_sources = pd.DataFrame(list(sources.items()), columns=["Data Element", "Source"])
                    st.table(df_sources)
